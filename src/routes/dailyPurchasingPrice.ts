...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  ...
  if (!list.length) return res.status(404).json({ message: 'Daily Purchasing Price not found' });
  return res.json({ message: 'Daily Purchasing Price fetched successfully', data: inflate(list[0]) });
});
...
router.post('/', authenticate, parseFields, async (req: Request, res: Response) => {
  ...
  const [result] = await pool.query(
    'INSERT INTO daily_purchasing_price (commodities_id, grade_id, date, price, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())',
  ...
  return res.status(201).json({ message: 'Daily Purchasing Price created successfully', data: inflate((rows as any[])[0]) });
});

router.put('/:id', authenticate, parseFields, async (req: Request, res: Response) => {
  const id = req.params.id;
  const [exists] = await pool.query('SELECT id FROM daily_purchasing_price WHERE id = ?', [id]);
  if (!(exists as any[]).length) return res.status(404).json({ message: 'Daily Purchasing Price not found' });
  ...
  if (keys.length) {
    ...
    await pool.query(
      `UPDATE daily_purchasing_price SET ${keys.map(k => `${k} = ?`).join(', ')} WHERE id = ?`,
      [...keys.map(k => updates[k]), id]
    );
  }
  const [rows] = await pool.query(SELECT_WITH_RELATIONS + ' WHERE d.id = ?', [id]);
  return res.json({ message: 'Daily Purchasing Price updated successfully', data: inflate((rows as any[])[0]) });
});

router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  const [result] = await pool.query('DELETE FROM daily_purchasing_price WHERE id = ?', [req.params.id]);
  if (!(result as any).affectedRows) return res.status(404).json({ message: 'Daily Purchasing Price not found' });
  return res.json({ message: 'Daily Purchasing Price deleted successfully' });
});
